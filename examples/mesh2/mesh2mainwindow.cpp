...
{
    QString s =
            QFileDialog::getOpenFileName(this, "", "../../data", "GridData Files (*.mes *.MES)");
    ...
    if (IO::load(dataWidget, s, ext)) {
        ...
        dimWidget->setText(QString("Cells ") + QString::number(a * b) + " (" + QString::number(a)
                           + "x" + QString::number(b) + ")");
...
void Mesh2MainWindow::createFunction(QString const &name)
{
    ...
    dimWidget->setText(QString("Cells ") + QString::number(a * b) + " (" + QString::number(a) + "x"
                       + QString::number(b) + ")");
...
void Mesh2MainWindow::setGrid(Qwt3D::SIDE s, bool b)
{
    ...
    dataWidget->coordinates()->setGridLines(sum != Qwt3D::NOSIDEGRID, sum != Qwt3D::NOSIDEGRID,
                                            sum);
    dataWidget->update();
}
...
{
    QString s = QFileDialog::getOpenFileName(this, "", "./../../data/colormaps",
                                             "Colormap files (*.map *.MAP)");
    adaptDataColors(s);
}
...
void Mesh2MainWindow::showRotate(double x, double y, double z)
{
    rotateLabel->setText(" Angles (" + QString::number(x, 'g', 3) + " ,"
                         + QString::number(y, 'g', 3) + " ," + QString::number(z, 'g', 3) + ")");
}
...
{
    scaleLabel->setText(" Scales (" + QString::number(x, 'g', 3) + " ," + QString::number(y, 'g', 3)
